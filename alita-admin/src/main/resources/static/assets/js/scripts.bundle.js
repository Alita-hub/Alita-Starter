...
var AlitaRequest = (function () {
  // 创建一个 Axios 实例
  const axiosInstance = axios.create({
    baseURL: 'http://127.0.0.1:8080', // 设置默认的请求路径
    headers: {
      // 设置默认的 Authorization 请求头
    }
  });
  ...
  axiosInstance.interceptors.request.use(
    config => {
      const token = AlitaCookie.get("jwtToken"); 
      if (token) {
...
var AlitaCookie = {
  ...
  set: function (name, value, options) {
    ...
    // 如果 expires 属性是 Date 对象，则将其转换为 UTC 字符串
    ...
    // 将选项中的属性添加到 cookie 字符串中
    for (var key in options) {
      if (options.hasOwnProperty(key)) {
        cookieString += '; ' + key;
        var optionValue = options[key];
        // 如果属性值不为 true，则将其添加到 cookie 字符串中
        if (true !== optionValue) {
  ...
  remove: function (name) {
    // 将指定名称的 cookie 值设置为空字符串，并将 max-age 属性设置为 -1，立即过期并删除
...
var AlitaUtil = (function () {
  ...
  return {
    ...
    parseJWT: function (token) {
      // 分割 JWT 字符串为头部、负载和签名部分
      const [header, payload, signature] = token.split('.');

      // 解码负载部分
      const decodedPayload = atob(payload);

      // 将解码后的负载部分转换为 JSON 对象
      const payloadObj = JSON.parse(decodedPayload);

      // 提取负载中的过期时间 (exp 属性)
      const expTimestamp = payloadObj.exp;

      // 计算过期时间
      const expDate = new Date(expTimestamp * 1000); // 乘以 1000 是因为 JavaScript 中的时间戳为毫秒级
      const now = new Date();
      const timeUntilExpiration = expDate - now;
